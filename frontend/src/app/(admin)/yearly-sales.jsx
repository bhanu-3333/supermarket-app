        ...
        <TouchableOpacity style={styles.calendarBox} activeOpacity={1} onPress={() => {}}>
          ...
          <View style={styles.calGrid}>
            {cells.map((day, i) => (
              <TouchableOpacity
                key={i}
                style={[styles.calCell, day === selected ? styles.calCellSelected : null]}
                onPress={() => day && setSelected(day)}
                disabled={!day}
              >
                <Text style={[styles.calCellText, day === selected ? styles.calCellTextSelected : null]}>
                  {day || ''}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              onFilter(new Date(viewYear, viewMonth, selected));
              onClose();
            }}
          >
...
function FilterModal({ visible, onClose, onApply }) {
  ...
  const opts = [
    { key: 'sales', label: 'Yearly sales', val: 'yearly' },
    { key: 'sales', label: 'Monthly sales', val: 'monthly' },
    { key: 'sales', label: 'Today sales', val: 'today' },
    { key: 'customer', label: 'Yearly customer', val: 'yearly' },
    { key: 'customer', label: 'Monthly customer', val: 'monthly' },
    { key: 'customer', label: 'Today customer', val: 'today' },
  ];
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={onClose}>
        <TouchableOpacity style={styles.filterBox} activeOpacity={1} onPress={() => {}}>
          ...
          {opts.filter(o => o.key === 'sales').map(o => (
            <TouchableOpacity
              key={o.val}
              style={styles.filterRow}
              onPress={() => { setSales(o.val); setCustomer(''); }}
            >
          ...
          {opts.filter(o => o.key === 'customer').map(o => (
            <TouchableOpacity
              key={o.val}
              style={styles.filterRow}
              onPress={() => { setCustomer(o.val); setSales(''); }}
            >
              <Text style={styles.filterRowText}>{o.label}</Text>
...
export default function YearlySalesScreen() {
  ...
  const fetchYearlySales = async () => {
    ...
    try {
      ...
      if (data.success && data.data.salesByYear) {
        const years = Object.keys(data.data.salesByYear).sort();
        setChartData(years.map((y, i) => ({ value: data.data.salesByYear[y], label: y, frontColor: COLORS[i % COLORS.length] })));
        setLegend(years.map((y, i) => ({ year: y, value: data.data.salesByYear[y], color: COLORS[i % COLORS.length] })));
      }
  ...
  const handleFilter = ({ sales, customer }) => {
    if (sales === 'yearly') return; // already here
    if (sales === 'monthly') router.replace('/(admin)/sales');
    if (customer === 'yearly') router.replace('/(admin)/yearly-customer');
    if (customer === 'monthly') router.replace('/(admin)/sales');
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.chartCard}>
          <Text style={styles.chartTitle}>Yearly sales</Text>
          {loading ? (
